...
# ============================
# Načtení signálů se spinnerem
# ============================
with st.spinner("Načítám signály, počkejte..."):
...
if not signals:
    ...
    # Doplnit chybějící sloupce, aby nevznikl KeyError
    expected_columns = ['instrument', 'price', 'signal', 'SL', 'TP', 'probability', 'profit_CZK', 'ticker']
    for col in expected_columns:
        if col not in df_signals.columns:
            df_signals[col] = None

    df_display = df_signals[expected_columns[:-1]]  # všechny kromě tickeru
    ...
    if not selected_row.empty:
        ...
        if ticker:  # pokud ticker není None
